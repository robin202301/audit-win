...
export const STAGE_FORM_CONFIGS: Record<string, StageFormConfig> = {
  ...
  survey_assessment: {
    ...
    fields: [
      { key: 'unitName', label: '单位名称' },
      { key: 'assessmentContent', label: '评估内容', type: 'textarea', rows: 10, fullSpan: true, placeholder: '评估被审计单位是否存在重要问题的可能性及依据' },
      { key: 'assessmentConclusion', label: '评估结论', type: 'textarea', rows: 4, fullSpan: true },
      { key: 'assessorName', label: '评估人' },
      { key: 'assessmentDate', label: '评估日期', type: 'date' },
    ],
  },
  survey_measures: {
    title: '确定审计事项和审计应对措施',
    ...
    fields: [
      { key: 'unitName', label: '单位名称' },
      { key: 'auditMatters', label: '审计事项', type: 'textarea', rows: 6, fullSpan: true, placeholder: '根据调查了解确定的重点审计事项' },
      { key: 'auditMeasures', label: '审计应对措施', type: 'textarea', rows: 8, fullSpan: true, placeholder: '针对各审计事项拟采取的应对措施和方法' },
      { key: 'fillerName', label: '填表人' },
      { key: 'fillDate', label: '填报日期', type: 'date' },
    ],
  },
