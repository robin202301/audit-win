...
onMounted(async () => {
  ...
  if (props.projectInfo) {
    if (!form.value.auditedUnit) {
      form.value.auditedUnit = props.projectInfo.auditedTarget;
    }
    if (!form.value.auditProject) {
      form.value.auditProject = props.projectInfo.name;
    }
  }
  try {
...
async function handleExport(): Promise<void> {
  try {
    ...
    if (res.success && res.data) {
      const data = {
        ...form.value,
        content: form.value.problemsFound || '',
        text: '',
        name: '',
        val: '',
        shortText: '',
      };
      const genRes = await window.electronAPI.documents.generate('tpl_final_report', data, res.data.filePath);
      if (genRes.success) {
